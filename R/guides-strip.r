...
labels_default <- function(plot, strip = plot$striplabel) {
  ...
  labels_grobs <- unlist(compact(list(
    if (ncol(gm) > 1) plot_grob_matrix(labels_h),
    if (nrow(gm) > 1) plot_grob_matrix(labels_v)
  )), recursive=FALSE)
...
ggstrip <- function(text, horizontal=TRUE, strip.gp=ggopt()$strip.gp, text.gp=ggopt()$strip.text.gp) {
	gTree(children = gList(
		rectGrob(gp=strip.gp, name=grobName(NULL,"strip-background")),
		textGrob(text, rot=-90 * (1 - horizontal), gp=text.gp, name=grobName(NULL,"strip-text")) 
	), name=grobName(NULL,"strip"))
}